...
ROWS = [
    ("host",),
    ("kv", "Now", "Backend Software Engineer @ Grip Shipping"),
    ("kv", "Prev", "Programming Lab Assistant @ UNF"),
    ("kv", "Also", "Data Analysis Intern @ SalesLand Colombia"),
    ("kv", "Edu", "B.S. CS, University of North Florida '25"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Backend", "Python, FastAPI, Jinja2, REST APIs"),
    ("kv", "Frontend", "JavaScript, Lit"),
    ("kv", "Data", "SQL Server, Redis, DuckDB, Databricks"),
    ("kv", "Tooling", "GitHub Actions, Azure App Insights, Git"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Built bulk ASN & order-feedback workflows in prod"),
    ("bul", "GPA 3.75/4.0 · President's & Dean's List"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">alejandro@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">alejandro</tspan><tspan fill="{MUTED}">@</tspan>'
